#!/usr/bin/env python3
"""
Analytical AI Agent - Streamlit Multi-CSV Query Interface
...
if 'vector_cache_dir' not in st.session_state:
    st.session_state.vector_cache_dir = Path("data/vector_cache")
    st.session_state.vector_cache_dir.mkdir(parents=True, exist_ok=True)
...
def get_cache_path(file_hash, filename):
    """Get path for cached vector data"""
    cache_dir = st.session_state.vector_cache_dir
    safe_filename = re.sub(r'[^\w\-_.]', '_', filename)
    return cache_dir / f"{safe_filename}_{file_hash}.pkl"

def save_vector_cache(file_hash, filename, file_id, metadata):
    """Save vectorized data to cache"""
    try:
        cache_path = get_cache_path(file_hash, filename)
        cache_data = {
            'file_id': file_id,
            'metadata': metadata,
...
def load_vector_cache(file_hash, filename):
    """Load vectorized data from cache"""
    try:
        cache_path = get_cache_path(file_hash, filename)
...
def load_csv_file(uploaded_file, clean_data=True):
    ...
    try:
        ...
        # Check if vectors are already cached
        cached_data = load_vector_cache(file_hash, uploaded_file.name)
        ...
        # Ingest CSV (this creates vectors)
        file_id, metadata = csv_ingestion.ingest_csv(tmp_path, vectorize=True)
        
        # Save vectors to cache
        save_vector_cache(file_hash, uploaded_file.name, file_id, metadata)
...
def main():
    ...
    with st.sidebar:
        ...
        if cache_files:
            with st.expander("💾 Vector Cache", expanded=False):
                st.write(f"**Cached files:** {len(cache_files)}")
                total_size = sum(f.stat().st_size for f in cache_files) / 1024 / 1024
                st.write(f"**Cache size:** {total_size:.2f} MB")
                ...
                with st.spinner("Loading files..."):
                    ...
                    for idx, file in enumerate(uploaded_files):
                        ...
                        if result.get('cached'):
                            status_text.text(f"✅ Loaded from cache: {file.name}")
                        
                        progress_bar.progress((idx + 1) / len(uploaded_files))
                
                success_count = sum(1 for f in st.session_state.loaded_files if f['status'] == 'success')
                cached_count = sum(1 for f in st.session_state.loaded_files if f.get('cached'))
                
                if success_count > 0:
                    msg = f"✅ Loaded {success_count} file(s)"
                    if cached_count > 0:
                        msg += f" ({cached_count} from cache)"
                    st.success(msg)
                st.rerun()
        ...
        if st.session_state.loaded_files:
            ...
            for file_info in st.session_state.loaded_files:
                if file_info['status'] == 'success':
                    # Determine icon based on cache status
                    icon = "💾" if file_info.get('cached') else "✅"
                    with st.expander(f"{icon} {file_info['name']}"):
                        st.write(f"**Rows:** {file_info['metadata'].num_rows}")
                        st.write(f"**Columns:** {file_info['metadata'].num_columns}")
                        
                        if file_info.get('cached'):
                            st.success(f"📦 Loaded from cache (saved API calls)")
                            if 'cache_timestamp' in file_info:
                                st.caption(f"Cached: {file_info['cache_timestamp'].strftime('%Y-%m-%d %H:%M')}")
                        else:
                            st.info("🆕 Newly vectorized and cached")
    ...
    if not st.session_state.loaded_files:
        ...
        st.subheader("💾 Vector Caching")
        st.markdown("""
        **Save API calls and time!**
        - Embeddings are automatically cached locally
        - Reloading the same file uses cached vectors
        - No repeated API calls for the same data
        - Cache is based on file content (not just name)
        """)
    else:
